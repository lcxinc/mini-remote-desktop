use crate::core::metrics::{Series, SeriesSummary};
...
pub struct PipelineConfig {
    ...
    pub stages: PipelineStageConfig,
}
...
impl Default for PipelineConfig {
    fn default() -> Self {
        Self {
            ...
            chain_name: "unknown_chain".to_string(),
            stages: PipelineStageConfig::default(),
        }
    }
}
...
pub struct PipelineReport {
    ...
    pub height: u32,
    pub target_fps: u32,
    pub bitrate_avg_mbps: f64,
    pub bitrate_p95_mbps: f64,
    pub wire_overhead_est_mbps: f64,
    pub frames_total: u64,
    pub frames_received: u64,
    pub frames_dropped: u64,
    pub capture: SeriesSummary,
    pub encode: SeriesSummary,
    pub send: SeriesSummary,
    pub recv_wait: SeriesSummary,
    pub decode: SeriesSummary,
    pub render: SeriesSummary,
    pub present_call: SeriesSummary,
    pub e2e: SeriesSummary,
    pub pass: bool,
...
pub fn run_udp_pipeline(cfg: PipelineConfig) -> Result<PipelineReport, String> {
                ...
                Ok((n, _)) => {
                    ...
                    if let Some(pkt) = FramePacket::decode(&buf[..n]) {
                        ...
                        if sec_window_start.elapsed() >= Duration::from_secs(1) {
                            ...
                            eprintln!(
                                "[DECODE-RUNTIME] transport=udp chain={} fps={:.2}",
                                chain_name, fps
                            );
                            sec_count = 0;
                            sec_bytes = 0;
    ...
    let e2e = rx_thread
        .join()
        .map_err(|_| "rx thread join failed".to_string())?;
    let (
        frames_received,
        frames_dropped,
        recv_wait,
        ...
        .recv()
        .map_err(|_| "report recv failed".to_string())?;
    let sec_total = run_start.elapsed().as_secs_f64().max(1e-6);
    let bitrate_avg_mbps = (rx_bytes as f64 * 8.0) / sec_total / 1_000_000.0;
    let wire_overhead_est_mbps = (frames_received as f64 * 28.0 * 8.0) / sec_total / 1_000_000.0;
...
pub(crate) fn p95_from_samples(mut samples: Vec<f64>) -> f64 {
    if samples.is_empty() {
        return 0.0;
    }
