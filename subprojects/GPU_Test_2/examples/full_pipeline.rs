...
async fn main() -> Result<()> {
    ...
    println!("================");
    println!();
    println!("链路：采集 -> 编码 -> QUIC 传输 -> 解码 -> 渲染");
    println!("目标：200+ FPS");
    println!();
    ...
    while running {
        ...
        match acquire_result {
            Ok(()) => {
                if let Some(resource) = frame_resource {
                    ...
                    let encoded_data = &rgb_data; // 模拟编码
                    ...
                    bytes_sent += rgb_data.len() as u64;
        ...
        if last_report.elapsed() >= Duration::from_secs(1) {
            ...
            let mbps = (bytes_sent as f64 * 8.0) / (last_report.elapsed().as_secs_f64() * 1_000_000.0);
            
            let avg_capture = capture_time.as_secs_f64() * 1000.0 / total_frames as f64;
            let avg_encode = encode_time.as_secs_f64() * 1000.0 / total_frames as f64;
            let avg_send = send_time.as_secs_f64() * 1000.0 / total_frames as f64;

            println!("帧率: {:.1} fps, 带宽: {:.2} Mbps", fps, mbps);
            println!("  采集延迟: {:.2} ms", avg_capture);
            println!("  编码延迟: {:.2} ms", avg_encode);
            println!("  传输延迟: {:.2} ms", avg_send);
            println!();
...
fn create_window() -> Result<HWND, Box<dyn std::error::Error>> {
    let class_name = PCSTR::from_raw("FullPipelineClass\0".as_ptr());
    ...
    let hwnd = unsafe {
        CreateWindowExA(
            Default::default(),
            class_name,
            PCSTR::from_raw("Full Pipeline\0".as_ptr()),
