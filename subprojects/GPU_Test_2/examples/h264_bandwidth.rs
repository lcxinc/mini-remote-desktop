...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 创建渲染纹理（窗口大小，用于缩放渲染）
    let render_texture = create_render_texture(&device, window_width, window_height)?;

    // 创建渲染暂存纹理（用于 Map/Unmap）
    ...
    let mut rendered_data = vec![0u8; window_width as usize * window_height as usize * 4]; // RGBA
    ...
    println!();
    println!("开始 H.264 编码测试...");
    println!("按 Ctrl+C 退出");
    println!("窗口显示桌面画面（缩放到窗口大小）");
    println!();
        ...
        match acquire_result {
            Ok(()) => {
                if let Some(resource) = frame_resource {
                    ...
                    // 使用 Map/Unmap 方法写入渲染纹理（与 simple_render_test 相同）
                    // 映射暂存纹理
                    let mut mapped = D3D11_MAPPED_SUBRESOURCE::default();
                    unsafe {
                        context.Map(
                    ...
                    // 复制数据到暂存纹理（RGBA 转 BGRA）
                    let data = unsafe {
                        std::slice::from_raw_parts_mut(
                            mapped.pData as *mut u8,
                            (mapped.RowPitch as usize) * (window_height as usize),
                        )
                    };

                    for y in 0..window_height as usize {
                        let src_row_start = y * window_width as usize * 4;
                        let dst_row_start = y * mapped.RowPitch as usize;

                        for x in 0..window_width as usize {
                            let src_idx = src_row_start + x * 4;
                            let dst_idx = dst_row_start + x * 4;

                            if src_idx + 3 < rendered_data.len() && dst_idx + 3 < data.len() {
                                // RGBA 转 BGRA
                                data[dst_idx] = rendered_data[src_idx + 2]; // B
                                data[dst_idx + 1] = rendered_data[src_idx + 1]; // G
                                data[dst_idx + 2] = rendered_data[src_idx]; // R
                                data[dst_idx + 3] = rendered_data[src_idx + 3]; // A
                            }
                        }
                    }
                    ...
                    // 复制到交换链后备缓冲区（直接复制，与 desktop_render_test 相同）
    ...
    drop(processed_tx);
    drop(re_use_tx);
    let (total_bytes, total_frames) = encode_thread.join().unwrap();

    // 输出最终结果
    let total_elapsed = start_time.elapsed();
    let avg_fps = frames_tested as f64 / total_elapsed.as_secs_f64();
    let avg_bps = (total_bytes as f64 * 8.0) / total_elapsed.as_secs_f64();
    let avg_encode_time = if encode_count > 0 {
        encode_time_total / encode_count as u32
    } else {
        Duration::ZERO
    };
    let avg_bytes_per_frame = if total_frames > 0 {
    ...
    println!("目标比特率: {} Mbps", BITRATE / 1_000_000);
    println!();
    ...
    println!("| 平均码率 | {:.2} Mbps |", avg_bps / 1_000_000.0);
    println!("| 平均每帧大小 | {:.2} KB |", avg_bytes_per_frame / 1024.0);
    println!(
        "| 平均编码延迟 | {:.3} ms |",
        avg_encode_time.as_secs_f64() * 1000.0
    );
    println!();

    // 输出带宽对比
    let raw_bps = (desktop_width * desktop_height * 4) as f64 * 8.0 * avg_fps;
    println!("=== 带宽对比 ===");
    println!("| 类型 | 带宽 |");
    println!("|------|------|");
    println!("| 原始数据 (RGBA) | {:.2} Mbps |", raw_bps / 1_000_000.0);
    println!("| H.264 编码后 | {:.2} Mbps |", avg_bps / 1_000_000.0);
    println!("| 压缩率 | {:.1}% |", (avg_bps / raw_bps) * 100.0);
    println!();
...
fn create_window() -> Result<(HWND, u32, u32), Box<dyn std::error::Error>> {
    let class_name = PCSTR::from_raw("H264BandwidthClass\0".as_ptr());
    ...
    let hwnd = unsafe {
        CreateWindowExA(
            Default::default(),
            class_name,
            PCSTR::from_raw("H.264 Bandwidth Test\0".as_ptr()),
...
fn create_staging_texture_for_write(
    ...
    let desc = D3D11_TEXTURE2D_DESC {
        ...
        MipLevels: 1,
        ArraySize: 1,
        Format: DXGI_FORMAT_B8G8R8A8_UNORM, // 与交换链格式一致
        SampleDesc: DXGI_SAMPLE_DESC {
            Count: 1,
            Quality: 0,
        },
