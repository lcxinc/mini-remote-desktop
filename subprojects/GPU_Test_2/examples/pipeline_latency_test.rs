...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut encoded_data = vec![0u8; WINDOW_WIDTH * WINDOW_HEIGHT * 3];
    let mut decoded_data = vec![0u8; WINDOW_WIDTH * WINDOW_HEIGHT * 3];
    let mut rendered_data = vec![0u8; WINDOW_WIDTH * WINDOW_HEIGHT * 3];
    ...
    let mut decode_stats = LatencyStats::new();
        ...
        match acquire_result {
            Ok(()) => {
                if let Some(resource) = frame_resource {
                    ...
                    encoded_data.copy_from_slice(&rgb_data);
                    let encode_time = encode_start.elapsed();
                    encode_stats.add(encode_time);

                    // 3. 解码阶段（模拟：拷贝数据）
                    let decode_start = Instant::now();
                    decoded_data.copy_from_slice(&encoded_data);
                    let decode_time = decode_start.elapsed();
                    decode_stats.add(decode_time);

                    // 4. 渲染阶段（模拟：拷贝数据）
                    let render_start = Instant::now();
                    rendered_data.copy_from_slice(&decoded_data);
    ...
    if capture_p95.as_secs_f64() * 1000.0 < 5.0 {
        println!("✅ 采集延迟良好");
    }
...
fn create_window() -> Result<HWND, Box<dyn std::error::Error>> {
    let class_name = PCSTR::from_raw("PipelineLatencyTestClass\0".as_ptr());
    ...
    let hwnd = unsafe {
        CreateWindowExA(
            Default::default(),
            class_name,
            PCSTR::from_raw("Pipeline Latency Test\0".as_ptr()),
