...
pub fn run_quic_datagram_pipeline(cfg: PipelineConfig) -> Result<PipelineReport, String> {
    let rt = tokio::runtime::Builder::new_multi_thread()
        .enable_io()
        .enable_time()
        .build()
        .map_err(|e| format!("build tokio runtime: {e}"))?;
    rt.block_on(run_quic_datagram_pipeline_async(cfg))
}

async fn run_quic_datagram_pipeline_async(cfg: PipelineConfig) -> Result<PipelineReport, String> {
    ...
    if client_conn.max_datagram_size().is_none() {
        return Err("quic_datagram_not_supported".to_string());
    }
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            match server_conn.read_datagram().await {
                Ok(buf) => {
                    rx_bytes = rx_bytes.saturating_add(buf.len() as u64);
                    sec_bytes = sec_bytes.saturating_add(buf.len() as u64);
                    if let Some(pkt) = FramePacket::decode(&buf) {
                        ...
                        if sec_window_start.elapsed() >= Duration::from_secs(1) {
                            ...
                            eprintln!(
                                "[DECODE-RUNTIME] transport=quic_datagram chain={} fps={:.2}",
                                chain_name, fps
                            );
                            sec_count = 0;
                            sec_bytes = 0;
    ...
    while run_start.elapsed() < Duration::from_secs(cfg.duration_sec) {
        ...
        client_conn
            ...
            .map_err(|e| format!("quic send_datagram: {e}"))?;
    ...
    let (
        ...
        .await
        .map_err(|_| "quic_datagram recv report channel closed".to_string())?;
...
fn make_quic_configs() -> Result<(ServerConfig, ClientConfig), String> {
    ...
    client_transport.datagram_send_buffer_size(64 * 1024);
