...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut encoded_data = vec![0u8; WINDOW_WIDTH * WINDOW_HEIGHT * 3 / 2];
    ...
    if capture_p95.as_secs_f64() * 1000.0 < 5.0 && encode_p95.as_secs_f64() * 1000.0 < 5.0 {
        println!("✅ 各阶段延迟良好");
    }
...
fn create_window() -> Result<HWND, Box<dyn std::error::Error>> {
    let class_name = PCSTR::from_raw("FullLatencyTestClass\0".as_ptr());
    ...
    let hwnd = unsafe {
        CreateWindowExA(
            Default::default(),
            class_name,
            PCSTR::from_raw("Full Latency Test\0".as_ptr()),
